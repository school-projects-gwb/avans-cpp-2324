...
void Sector::MoveObjects(const enums::SectorObjectType &object_type, const Coords &target_location) {
  ...
  for (int x = 0; x < objects_.GetColCount(); ++x) {
    for (int y = 0; y < objects_.GetRowCount(); ++y) {
      ...
      if (objects_[current_position] == object_type) {
        objectsToMove.push_back(current_position);
      }
    }
  }

  for (const Coords &current_position : objectsToMove) {
    Coords next_position = CalculateNextPosition(current_position, target_location);
    if (IsValidMove(next_position, object_type)) {
      MoveObject(current_position, next_position);
    }
  }
}
...
void Sector::MoveObjectAtPositionToTargetPosition(const Coords &current_position, const Coords &target_position) {
  if (IsValidMove(target_position, objects_[current_position]))
    MoveObject(current_position, target_position);
}
...
const VectorGrid<enums::SectorObjectType>& Sector::GetSectorObjects() const {
  return objects_;
}
...
bool Sector::IsValidMove(const Coords &next_position, enums::SectorObjectType object_type) const {
  return IsPositionInSectorBounds(next_position) && objects_[next_position] == enums::SectorObjectType::EmptySpace;
}

void Sector::MoveObject(const Coords &current_position, const Coords &next_position) {
  objects_[next_position] = objects_[current_position];
...
Coords Sector::CalculateNextPosition(const Coords &current_position, const Coords &target_location) {
